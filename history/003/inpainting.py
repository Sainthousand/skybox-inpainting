...
def generate_image_sequencce(init_image_list, mask_image_list, output_path):
    ...
    # prompt = "(sky), (pure), high qulity, (real)"
    # negative_prompt = "car, (text), (destroyed), (bridge), (building), paper, plane, boat, (artifact), tower, error, sketch, horror, geometry, stone, cartoon, slogan, ground, tree, blurred, ceiling, branches, pixelated, Disfigured, cartoon, nude, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry,"
...
def generate_image(init_image, mask_image, output_path, idx_str):
    pipeline = AutoPipelineForInpainting.from_pretrained(
    # pipeline = StableDiffusionXLPipeline.from_pretrained(
        "diffusers/stable-diffusion-xl-1.0-inpainting-0.1",
        torch_dtype=torch.float16, 
    ...
    mask_blurred = pipeline.mask_processor.blur(mask_image, blur_factor=25)
...
def refine_mask(mask, idx_str):
    ...
    new_mask.save(f"/home/shengqian.li/code/skybox_inpainting/mask_refined/{idx_str}.png")
...
if __name__=="__main__":
    ...
    for idx in tqdm(range(1, 100)):
        ...
        # refine mask and img
        mask_image = refine_mask(mask_image, idx_str)
        ...
        wb_img.save(f"/home/shengqian.li/code/skybox_inpainting/wb_img/{idx_str}.png")
    ...
    generate_image_sequencce(wb_img_list, wb_mask_list, output_path)
